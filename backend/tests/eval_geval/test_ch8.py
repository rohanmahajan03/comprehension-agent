...
_Q10_POINTS = [
    'A time-of-day clock returns the current date and time, synchronized via NTP, but can jump, so it is unsuitable for measuring elapsed time.',
    'A monotonic clock only moves forward, so it is suited to measuring durations such as timeouts.',
]
